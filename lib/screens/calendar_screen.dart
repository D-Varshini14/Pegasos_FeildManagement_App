...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Future<void> _loadVisits() async {
    try {
      ...
      final visitRes = results[0];
      final taskRes = results[1];

      List<summary.Visit> combined = [];

      if (visitRes['success'] == true && visitRes['data'] != null) {
        final List data = visitRes['data'];
        combined.addAll(data.map((e) => summary.Visit.fromJson(e as Map<String, dynamic>)));
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Calendar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
        centerTitle: true,
      ),
      drawer: _buildSidebar(context),
      body: Column(
                                    ...
                                    if (_currentDate.month != DateTime.now().month || _currentDate.year != DateTime.now().year)
                                      InkWell(
                                        ...
                                        child: Container(
                                          ...
                                          child: const Text('Today', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: primaryBlue)),
                                        ),
                                      ),
                                  ],
                                ),
