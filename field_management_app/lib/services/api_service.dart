...
//
//   // Production: use your EC2 public DNS
//   static const String baseUrl = 'http://ec2-52-63-22-74.ap-southeast-2.compute.amazonaws.com:3000/api';
//
//
//   // Login method
//   static Future<Map<String, dynamic>> login(
...
class ApiService {
  // Production: EC2 public IP (change to DNS if you have one)
  static const String baseUrl = 'http://52.63.22.74:3000/api';
  ...
  static Future<Map<String, dynamic>> login(
    ...
    try {
      printDebug('Attempting login for user: $name, userId: $userId');
      ...
      printDebug('Login response status: ${response.statusCode}');
      printDebug('Login response body: ${response.body}');
  ...
  static Future<Map<String, dynamic>> signup({
    ...
    try {
      printDebug('Attempting signup for user: $name, email: $email');
      ...
      printDebug('Signup response status: ${response.statusCode}');
      printDebug('Signup response body: ${response.body}');

      return jsonDecode(response.body);
    } catch (e) {
      printDebug('Signup error: $e');
      return {
        'success': false,
        'message': 'Network error: ${e.toString()}',
      };
  ...
  static Future<Map<String, dynamic>> getUserProfile() async {
    try {
      ...
      printDebug('Fetching user profile with token');

      final response = await http.get(
        Uri.parse('$baseUrl/user/profile'),  // Fixed: was missing /api prefix
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      printDebug('Get profile response status: ${response.statusCode}');

      return jsonDecode(response.body);
    } catch (e) {
      printDebug('Get profile error: $e');
      return {
        'success': false,
        'message': 'Network error: ${e.toString()}',
      };
  ...
  static Future<Map<String, dynamic>> testConnection() async {
    try {
      printDebug('Testing connection to: $baseUrl');

      final response = await http.get(
        Uri.parse('http://52.63.22.74:3000/'),  // Health check endpoint
      ).timeout(Duration(seconds: 10));

      printDebug('Connection test response: ${response.statusCode}');
      printDebug('Connection test body: ${response.body}');
      ...
      printDebug('Connection test error: $e');
      return {
        'success': false,
